...
(function initAutoFitButtons(){
    try{
    const MIN_PX = 8; // 可読性を保つ下限（より収まり優先）
...
async function initMic(allowPrompt=false){
    ...
    try{
        ...
        let stream = null;
        try{
            stream = await navigator.mediaDevices.getUserMedia({ audio: rawAudio, video:false });
        }catch(e1){
            // 互換性フォールバック（単純指定）
...
vOffsetSliderRight && (vOffsetSliderRight.oninput=()=>{ verticalOffset=parseInt(vOffsetSliderRight.value); syncVerticalOffsetSliders(); drawChart(); });
