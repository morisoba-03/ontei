...
let verticalZoom=3,verticalOffset=0,pxPerSec=100,timelineOffsetSec=0,isPanning=false,panStartX=0,panStartOffset=0,panStartY=0,panStartVOffset=0,panStartScrollPx=0;
// ピクセルベース縦スクロール蓄積値（1pxドラッグ=1px表示移動を実現するための内部値）
let verticalScrollPx=0; // 0 を基準 (下方向ドラッグで増加)
// 縦スクロール更新を集約 (wheel / drag / touch)
let _rafVReq=false;
function adjustVerticalOffset(deltaPct){
    if(!deltaPct) return;
    const next = Math.max(0, Math.min(100, verticalOffset + deltaPct));
    if(next===verticalOffset) return;
    verticalOffset = next;
    // percent 変更に合わせて pixel 蓄積値を再計算 (ズレ抑制)
    if(chartCanvas){
        const h = chartCanvas.getBoundingClientRect().height || chartCanvas.height || 600;
        const visibleSemis = verticalZoom*12;
        const scrollableSemis = 96 - visibleSemis;
        const scrollablePx = (scrollableSemis>0)? (h * scrollableSemis / visibleSemis) : 0;
        verticalScrollPx = (verticalOffset/100) * scrollablePx;
    }
    if(!_rafVReq){
        _rafVReq=true;
        requestAnimationFrame(()=>{
            _rafVReq=false;
            try{ updateVerticalThumb(); }catch(_){ }
            drawChart();
            try{ setSelectionByPlayhead(); }catch(_){ }
        });
    }
}
...
// ガイドノートへの吸着（再生中のみ）を任意に切り替え（既定OFF）
let snapToGuideNotes=false;
...
const vOffsetSlider=$('verticalOffsetSlider'); // (旧UI) 現状DOMに存在しない想定
const vOffsetValue=$('verticalOffsetValue');
const timelineVScroll=$('timelineVScroll');
const timelineVThumb=$('timelineVThumb');
// 旧 range スライダ UI は撤去済み。存在する場合のみ同期するが新UIは timelineVScroll を使用。
if(vOffsetSlider){ vOffsetSlider.style.display='none'; }

// ================= Vertical Scrollbar (timelineVScroll) =================
function updateVerticalThumb(){
    if(!timelineVScroll || !timelineVThumb) return;
    // 可視範囲: verticalZoom * 12 半音 / 全体(固定レンジ = 36..132 で 96半音)
    const totalRange=96; // 36-132
    const visible = verticalZoom*12;
    const ratio = Math.min(1, visible/totalRange);
    const barHeight = timelineVScroll.clientHeight; // padding含む高さ
    const minThumb = 24; // 最小サイズ
    const thumbHeight = Math.max(minThumb, Math.round(barHeight*ratio - 8));
    timelineVThumb.style.height = thumbHeight + 'px';
    // verticalOffset 0..100 -> 上部余白最大値
    const maxTravel = barHeight - thumbHeight - 8; // margin上下 4px *2
    // 表示: verticalOffset 大 = 上側高音域をより表示 → ユーザー要望で『下へドラッグ=低音域へ』のため、thumb位置は逆マッピング
    const y = Math.round(((100-verticalOffset)/100)*maxTravel) + 4;
    timelineVThumb.style.transform = `translateY(${y}px)`;
}

let _vDrag=false,_vDragStartY=0,_vDragStartOffset=0;
if(timelineVScroll && timelineVThumb){
        const onWheel=(e)=>{
            e.preventDefault();
            // 下ホイール = 低音域へ = verticalOffset 減少
            const step = (e.deltaY>0? -5 : 5);
            verticalOffset = Math.max(0, Math.min(100, verticalOffset + step));
            updateVerticalThumb();
            drawChart();
            try{ setSelectionByPlayhead(); }catch(_){ }
        };
    timelineVScroll.addEventListener('wheel', onWheel, {passive:false});
    timelineVThumb.addEventListener('wheel', onWheel, {passive:false});
    timelineVThumb.addEventListener('mousedown',(e)=>{
        e.preventDefault(); _vDrag=true; _vDragStartY=e.clientY; _vDragStartOffset=verticalOffset; document.body.style.userSelect='none';
    });
    window.addEventListener('mousemove',(e)=>{
        if(!_vDrag) return;
        const barHeight = timelineVScroll.clientHeight;
        const totalRange=96; const visible=verticalZoom*12; const ratio=visible/totalRange; // 情報用途
        const thumbH = timelineVThumb.clientHeight;
        const maxTravel = barHeight - thumbH - 8;
        const dy = e.clientY - _vDragStartY; // dy>0 下ドラッグ
        // 下へドラッグ = 低音域へ = verticalOffset 減少
        const deltaPercent = (dy / Math.max(1,maxTravel))*100;
        verticalOffset = Math.max(0, Math.min(100, _vDragStartOffset - deltaPercent));
        try{ updateVerticalThumb(); }catch(_){ }
        drawChart();
        try{ setSelectionByPlayhead(); }catch(_){ }
    });
    window.addEventListener('mouseup',()=>{ if(_vDrag){ _vDrag=false; document.body.style.userSelect=''; }});
    // クリックジャンプ
    timelineVScroll.addEventListener('mousedown',(e)=>{
        if(e.target===timelineVThumb) return; // thumb自体のドラッグは既に処理
        const rect = timelineVScroll.getBoundingClientRect();
    const clickY = e.clientY - rect.top - timelineVThumb.clientHeight/2;
        const barHeight = timelineVScroll.clientHeight;
        const maxTravel = barHeight - timelineVThumb.clientHeight - 8;
        const pos = Math.max(0, Math.min(maxTravel, clickY - 4)); // 0 上端, maxTravel 下端
        // 下側クリック = 低音域側 = verticalOffset 減少 → 逆マッピング
        const pctFromTop = pos / Math.max(1,maxTravel); // 0..1
        verticalOffset = Math.max(0, Math.min(100, (1 - pctFromTop)*100));
        updateVerticalThumb();
        drawChart();
        try{ setSelectionByPlayhead(); }catch(_){ }
    });
    // 初期
    setTimeout(updateVerticalThumb, 50);
    window.addEventListener('resize', ()=> updateVerticalThumb());
}

// verticalZoom を変更する別UIから呼ばれるケースに備えてフック（後段で既存ハンドラに統合）
const _origApplyVerticalZoomChange = typeof applyVerticalZoomChange==='function'? applyVerticalZoomChange : null;
function applyVerticalZoomChangePatched(v){
    if(_origApplyVerticalZoomChange){ _origApplyVerticalZoomChange(v); }
    updateVerticalThumb();
}
// 既存の verticalZoom 変更処理が明示関数でなければ、slider の oninput を後で差し替える必要あり（下側で再定義されていれば適宜調整）。
const formatSelect=null; // 使用フォーマットUIは削除
...
async function extractMelodyNotesFromBuffer(buf){
    ...
    function yinBestTauCMND(frameArr){
        ...
        for(let tau=tauMin; tau<=tauMax; tau++){
            ...
            for(let n=0;n<M;n++){ const diff=frameArr[n]-frameArr[n+tau]; sum += diff*diff; }
            d[tau]=sum;
        }
        const cmnd=new Float32Array(tauMax+1);
        let acc=0;
        cmnd[0]=1; for(let tau=1; tau<=tauMax; tau++){ acc += d[tau]; const mean=acc/tau; cmnd[tau] = mean>1e-12? (d[tau]/mean) : 1; }
        let bestTau=-1; let bestVal=1e9;
        const thresh=0.18;
        for(let tau=tauMin; tau<=tauMax; tau++){
            const v=cmnd[tau];
            if(v<thresh){ bestTau=tau; bestVal=v; break; }
            if(v<bestVal){ bestVal=v; bestTau=tau; }
        }
        // 近傍で補間（パラボリック）
        if(bestTau>tauMin && bestTau<tauMax){
            const ym1=cmnd[bestTau-1], y0=cmnd[bestTau], yp1=cmnd[bestTau+1];
            const denom = (ym1 - 2*y0 + yp1);
            if(Math.abs(denom)>1e-9){ const delta = 0.5*(ym1 - yp1)/denom; if(Math.abs(delta)<=1) bestTau = bestTau + delta; }
        }
        const conf = Math.max(0, Math.min(1, 1 - (isFinite(bestVal)? bestVal: 1)));
        return {tau: bestTau, conf};
    }
    ...
    const NACF_THRESH = 0.28; // やや緩めにして短音の取りこぼしを低減
    ...
    const estR = bestR;
        let freq = (estR>=NACF_THRESH && estTau>0)? (srd/estTau) : 0;
...
function seekTo(sec){
    ...
    playbackPosition=sec; playbackStartPos=sec; scheduleAll();
    if(isPlaying){ pausePlayback(); startPlayback(); } else drawChart();
    // シーク後も即自動選択
    try{ setSelectionByPlayhead(); }catch(_){ }
}
...
timelineScroll && (timelineScroll.oninput = () => {
    ...
    drawChart();
    // タイムライン移動後も編集モードなら即座に再生線下ノートを再選択
    try{ setSelectionByPlayhead(); }catch(_){ }
});
// ---- Markers ----
        ...
        if(snapToGuideNotes && isPlaying){
            try{
                ...
                if(tr && tr.notes && tr.notes.length){
                    ...
                    if(nn){
                        let mTar = nn.midi|0;
                        while(liveMidi - mTar > 6) liveMidi -= 12;
                        while(mTar - liveMidi > 6) liveMidi += 12;
                    }
                }
            }catch(_){ }
        }
...
// ---- Draw ----
function loop(){ updatePlaybackPositionHybrid(); drawChart(); setSelectionByPlayhead(); if(isPlaying) requestAnimationFrame(loop); }
// LRU 定期チェック (再生ループ駆動)
...
function drawChart(){
    ...
    const total=verticalZoom*12; const min=36; const maxDisplay=132;
    // verticalScrollPx を半音換算
    const hPx = h;
    const scrollableSemis = 96 - total;
    const pxPerSemiView = hPx/total;
    const scrollablePx = (scrollableSemis>0)? (pxPerSemiView*scrollableSemis) : 0;
    // verticalOffset(%) を pixel ベースから再導出（ドラッグ処理で直接 px 加算した場合の同期）
    if(scrollablePx>0){
        verticalOffset = Math.max(0, Math.min(100, (verticalScrollPx/scrollablePx)*100));
    } else {
        verticalOffset = 0;
        verticalScrollPx = 0;
    }
    const rawStart = (maxDisplay-min-total)*(verticalOffset/100);
    const vminFloat = min + rawStart;
    const vmin = Math.floor(vminFloat); // 描画ループ用整数基準
    const subSemiOffset = vminFloat - vmin; // 0..1 未満: 部分半音分のピクセルシフト
    const vmax = vmin + total;
    const pxSemi = h/total;
    const playX=getPlayX(w); const eff=playbackPosition+timelineOffsetSec;
    // 背景グリッド（白鍵=白、黒鍵=グレー）
    for(let m=vmin;m<=vmax;m++){
        const y=h-((m-vmin+1)-subSemiOffset)*pxSemi;
        ctx.beginPath();
        ctx.moveTo(0,y);
        ctx.lineTo(w,y);
        const pc = ((m%12)+12)%12;
    ...
    const visEnd  =eff + ((w-playX)/pxPerSec)*tempoFactor + visMarginSec;
        ...
        for(const n of notes){
            if(n.time>visEnd) break; // 前提: notes 時間順
            if(n.time+n.duration<visStart) continue;
            const x1=playX+(n.time-eff)*pxPerSec/tempoFactor; const x2=playX+((n.time+n.duration)-eff)*pxPerSec/tempoFactor; if(x2<0||x1>w) continue; const y=h-(n.midi-vmin+1)*pxSemi;
            // 選択判定
            const sel=window._selection||{type:'none'};
            let isSel=false;
            if(sel.type==='single' && sel.index!=null){ isSel = (notes[sel.index]===n); }
            else if(sel.type==='range' && sel.startSec!=null && sel.endSec!=null){ isSel = !(n.time>sel.endSec || (n.time+n.duration)<sel.startSec); }
            if(isSel){
                ctx.save();
                // 下地のハイライト帯
                ctx.fillStyle='rgba(255,210,78,0.28)';
                const barH = Math.min(10, pxSemi*0.9);
                ctx.fillRect(x1, y-barH/2, x2-x1, barH);
                // 枠線を太めに
                ctx.strokeStyle='#ffd24e'; ctx.lineWidth=Math.max(guideLineWidth,2.5);
                ctx.beginPath(); ctx.moveTo(x1,y); ctx.lineTo(x2,y); ctx.stroke();
                ctx.restore();
            }
            else { ctx.beginPath(); ctx.moveTo(x1,y); ctx.lineTo(x2,y); ctx.stroke(); }
    ...
    // キャリブレーション中は非表示
    if(!isCalibrating && pitchHistory.length && !isAssistActive()){
    ...
    if(_assistSegments && _assistSegments.length){
        ctx.save();
...
function setSelectionByPlayhead(){
    try{
        ...
        // 既に index 選択中でそのノートがまだ再生線下なら何もしない
        if(sel.type==='single' && sel.index!=null){
            const n=tr.notes[sel.index]; if(n && t>=n.time && t<=n.time+n.duration) return;
        }
        // 二分探索的に近くのノートを探す（線形でも十分小規模なので線形でも可）
        // ここでは線形: 最初に包含するもの
        let idx=-1;
    ...
    if(audition){
        try{
            ...
            if(tr&&tr.notes&&tr.notes[idx]){
                ...
                // サンプル再生のみを使用（フォールバックOSC削除）
                simplePlaySfz(m, (audioCtx?audioCtx.currentTime:0)+0.02, d, practiceCallGain||masterGain||audioCtx?.destination);
            }
        }catch(_){ }
    }
    drawChart();
...
if(chartCanvas){
    chartCanvas.addEventListener('wheel',(e)=>{
        ...
        e.preventDefault();
        const step = (e.deltaY>0? -5 : 5); // 下スクロールで下方向
        adjustVerticalOffset(step);
    }, {passive:false});
if(chartCanvas){ chartCanvas.onmousedown=e=>{ isPanning=true; panStartX=e.clientX; panStartOffset=timelineOffsetSec; panStartY=e.clientY; panStartVOffset=verticalOffset; panStartScrollPx=verticalScrollPx; }; }
const INVERT_VERTICAL_DRAG = true; // true: 下へドラッグで下方向スクロール
let VERTICAL_DRAG_SENSITIVITY = 1.0; // ピクセル1:1 を目指すため 1.0 （過度なら <1.0）
window.addEventListener('mousemove',e=>{ if(isPanning){
    const dx=e.clientX-panStartX; // 横パン: 時間方向
    const deltaTime=-dx/pxPerSec;
    timelineOffsetSec=Math.max(0,panStartOffset+deltaTime);
    // 縦パン: ピクセル→半音→percent 変換で 1px ≒ 1px の視覚移動
    const dy=e.clientY-panStartY;
    if(dy!==0 && chartCanvas){
    const h = chartCanvas.getBoundingClientRect().height || 600;
        const visibleSemis = verticalZoom*12;
        const scrollableSemis = 96 - visibleSemis;
        const pxPerSemi = h / visibleSemis;
    const deltaPxRaw = dy * (INVERT_VERTICAL_DRAG?1:-1) * VERTICAL_DRAG_SENSITIVITY; // 下へドラッグ=正
    // 加速度バグ防止: 開始時の scrollPx を基準に相対量で計算
    verticalScrollPx = Math.max(0, Math.min(scrollableSemis*pxPerSemi, panStartScrollPx + deltaPxRaw));
        if(scrollableSemis>0){
            const scrollablePx = pxPerSemi*scrollableSemis;
            const pct = (verticalScrollPx/scrollablePx)*100;
            if(pct!==verticalOffset){ verticalOffset=pct; try{ updateVerticalThumb(); }catch(_){ } }
        }
    }
    drawChart(); try{ setSelectionByPlayhead(); }catch(_){ }
} });
window.addEventListener('mouseup',()=>{ if(isPanning){ isPanning=false; } });
    // タッチパン (マウスと同一ロジック) -------------------------------------------------
    let touchPan=null; // {x,y, scrollPx, timeOff}
    const touchStart=(e)=>{
        if(!e.touches || e.touches.length!==1) return;
        const t=e.touches[0];
        touchPan={x:t.clientX,y:t.clientY, scrollPx:verticalScrollPx, timeOff:timelineOffsetSec};
    };
    const touchMove=(e)=>{
        if(!touchPan || !chartCanvas) return;
        const t=e.touches[0]; if(!t) return;
        const dx=t.clientX-touchPan.x; const dy=t.clientY-touchPan.y;
        // 横: 時間
        timelineOffsetSec=Math.max(0, touchPan.timeOff + (-dx/pxPerSec));
        // 縦: ピクセル1:1 (mouseと同じ計算)
        const h = chartCanvas.getBoundingClientRect().height || 600;
        const visibleSemis=verticalZoom*12; const scrollableSemis=96-visibleSemis; const pxPerSemi=h/visibleSemis;
        if(scrollableSemis>0){
            const limit=scrollableSemis*pxPerSemi;
            const deltaPxRaw = dy * (INVERT_VERTICAL_DRAG?1:-1) * VERTICAL_DRAG_SENSITIVITY;
            verticalScrollPx = Math.max(0, Math.min(limit, touchPan.scrollPx + deltaPxRaw));
            const pct = (verticalScrollPx/limit)*100;
            if(pct!==verticalOffset){ verticalOffset=pct; try{ updateVerticalThumb(); }catch(_){ } }
        }
        drawChart(); try{ setSelectionByPlayhead(); }catch(_){ }
        e.preventDefault();
    };
    const touchEnd=()=>{ touchPan=null; };
    chartCanvas.addEventListener('touchstart', touchStart, {passive:true});
    chartCanvas.addEventListener('touchmove', touchMove, {passive:false});
    chartCanvas.addEventListener('touchend', touchEnd, {passive:true});
    if(timelineVScroll){
        timelineVScroll.addEventListener('touchstart', touchStart, {passive:true});
        timelineVScroll.addEventListener('touchmove', touchMove, {passive:false});
        timelineVScroll.addEventListener('touchend', touchEnd, {passive:true});
    }
    ...
    chartCanvas.addEventListener('touchstart',(e)=>{
        const t=e.touches[0]; if(!t) return; const rect=chartCanvas.getBoundingClientRect(); const x=t.clientX-rect.left; const y=t.clientY-rect.top;
        if(_rangePending){ /* ignore */ }
    const idx=hitTestNote(x,y); if(idx!=null){ setSingleSelection(idx,{audition:true}); }
    },{passive:false});
    chartCanvas.addEventListener('touchmove',(e)=>{
        if(!draggingRange) return; const t=e.touches[0]; if(!t) return; const rect=chartCanvas.getBoundingClientRect(); const x=t.clientX-rect.left; const eff=playbackPosition+timelineOffsetSec; const playX=(chartCanvas&&chartCanvas.width)? getPlayX(chartCanvas.width):70; setRangeSelection(viewToTime(dragStartX,eff,playX), viewToTime(x,eff,playX)); e.preventDefault();
    },{passive:false});
    chartCanvas.addEventListener('touchend',()=>{ draggingRange=false; });
}
// 範囲開始/終了ボタン処理
...
if(rangeStartBtn){
    rangeStartBtn.onclick=()=>{
        try{
            // 起点は再生線下ノート。なければ自動選択を試みる
            setSelectionByPlayhead();
            const sel=window._selection; const tr=currentTracks[melodyTrackIndex];
            if(!(sel.type==='single' && sel.index!=null && tr&&tr.notes[sel.index])){ rangeHint.textContent='起点ノートを特定できません'; return; }
            const note=tr.notes[sel.index];
            _rangePending={ startIdx: sel.index, startTime: note.time };
            rangeHint.textContent='終点となるノートで「範囲終了」を押してください';
            if(rangeEndBtn) rangeEndBtn.disabled=false;
        }catch(_){ }
    };
}
if(rangeEndBtn){
...
function updatePlaybackPositionHybrid(){
    ...
    try{
        ...
        // しきい値: AudioContext の進みが実時間の 60% 未満に落ちたら perf ベースにフォールバック
        const usePerf = (perfDt>0.2 && ctxDt/perfDt < 0.6);
        const dt = usePerf? perfDt: ctxDt;
...
function autoCenterMelodyTrack(){
    ...
    // 選択トラックのノート群を見て、縦方向（verticalOffset）を“曲全体が収まるよう”に自動調整
    const tr=currentTracks[melodyTrackIndex]; if(!tr||!tr.notes||!tr.notes.length) return;
    ...
    if(vOffsetSlider){ vOffsetSlider.value = String(verticalOffset); }
}
...
// ---- UI ----
// 音声ファイル UI
if(melodyAudioBtn && melodyAudioInput){ melodyAudioBtn.onclick=()=>melodyAudioInput.click(); }
if(melodyAudioInput){ melodyAudioInput.onchange=async e=>{ const f=e.target.files?.[0]; if(!f) return; if(melodyAudioLabel){ melodyAudioLabel.textContent=f.name; melodyAudioLabel.title=f.name; } const overlay=document.getElementById('analyzingOverlay'); try{ if(overlay){ overlay.classList.remove('hidden'); } const ab=await f.arrayBuffer(); const srcU8=new Uint8Array(ab); // 保存用にディープコピー（decodeAudioDataでdetachされても安全）
    melodyOrigBytes=new Uint8Array(srcU8.length); melodyOrigBytes.set(srcU8);
...
vZoom && (vZoom.oninput=()=>{ verticalZoom=parseInt(vZoom.value); drawChart(); try{ updateVerticalThumb(); }catch(_){ } });
...
function resizeCanvas(){
    ...
    const parent=chartCanvas.parentElement; if(parent){
        ...
        // 左側の縦スライダーパネル分を差し引いて幅確保（横溢れ防止）
        let avail = Math.min(inner, Math.floor(vvw));
        try{
            const leftPanel = document.getElementById('leftVerticalPanel');
            if(leftPanel){ avail = Math.max(0, avail - leftPanel.getBoundingClientRect().width); }
        }catch(_){ }
    ...
    try{
        ...
        practiceModeOn.onclick=()=>{
            activateAudioOnce();
                    ...
                    practicePatternSelect && (practicePatternSelect.style.display='');
                    practiceRangeWrap && (practiceRangeWrap.style.display='inline-flex');
                    practiceStartBtn && (practiceStartBtn.style.display='');
                    practicePauseBtn && (practicePauseBtn.style.display='');
                    practiceStopBtn && (practiceStopBtn.style.display='');
                    practiceVolWrap && (practiceVolWrap.style.display='inline-flex');
